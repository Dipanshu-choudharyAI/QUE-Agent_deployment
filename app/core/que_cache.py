...
def _build_caches(settings: Settings) -> tuple[QueTtlLruCache, QueTtlLruCache, QueTtlLruCache]:
    max_entries = settings.que_cache_max_entries
    intent_ttl = settings.que_cache_intent_ttl_seconds
    llm_ttl = settings.que_cache_llm_ttl_seconds
    variant_ttl = settings.que_cache_variant_ttl_seconds
    return (
        QueTtlLruCache(max_entries=max_entries, default_ttl_seconds=intent_ttl),
        QueTtlLruCache(max_entries=max_entries, default_ttl_seconds=llm_ttl),
        QueTtlLruCache(max_entries=max_entries, default_ttl_seconds=variant_ttl),
    )


def reset_que_caches(*, settings: Settings | None = None) -> None:
    """Drop process caches (tests / config reload)."""
    global _intent_cache, _llm_cache, _variant_cache
    with _init_lock:
        if settings is None:
            _intent_cache = None
            _llm_cache = None
            _variant_cache = None
            return
        _intent_cache, _llm_cache, _variant_cache = _build_caches(settings)


def _caches() -> tuple[QueTtlLruCache, QueTtlLruCache, QueTtlLruCache] | None:
    global _intent_cache, _llm_cache, _variant_cache
    cfg = get_settings()
    if not cfg.que_cache_enabled:
        return None
    with _init_lock:
        if _intent_cache is None or _llm_cache is None or _variant_cache is None:
            _intent_cache, _llm_cache, _variant_cache = _build_caches(cfg)
        return _intent_cache, _llm_cache, _variant_cache
